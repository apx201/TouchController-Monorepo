...
object CrosshairRendererImpl : CrosshairRenderer {
    override fun renderOuter(canvas: Canvas, config: TouchRingConfig) {
        ...
        withShader({ GameRenderer.getPositionColorProgram()!! }) {
            ...
            for (i in 0 until CROSSHAIR_CIRCLE_PARTS) {
                ...
                bufferBuilder.vertex(matrix, point0.x, point0.y, 0f).color(Colors.WHITE.value).next()
                bufferBuilder.vertex(matrix, point2.x, point2.y, 0f).color(Colors.WHITE.value).next()
                bufferBuilder.vertex(matrix, point3.x, point3.y, 0f).color(Colors.WHITE.value).next()
                bufferBuilder.vertex(matrix, point1.x, point1.y, 0f).color(Colors.WHITE.value).next()
            }
    ...
    override fun renderInner(canvas: Canvas, config: TouchRingConfig, progress: Float) {
        ...
        withShader({ GameRenderer.getPositionColorProgram()!! }) {
            ...
            bufferBuilder.vertex(matrix, 0f, 0f, 0f).color(Colors.WHITE.value).next()
            ...
            for (i in 0..CROSSHAIR_CIRCLE_PARTS) {
                ...
                bufferBuilder.vertex(matrix, point.x, point.y, 0f).color(Colors.WHITE.value).next()
            }
